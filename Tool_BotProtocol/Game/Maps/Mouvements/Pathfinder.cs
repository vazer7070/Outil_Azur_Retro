...
namespace Tool_BotProtocol.Game.Maps.Mouvements
{
    public class Pathfinder : IDisposable
    {
        ...
        public List<Cell> GetPath(Cell CelluleInitiale, Cell CellFinale, List<Cell> ForbidenCell, bool D, byte distance, Map M = null)
        {
            ...
            {
                int index = 0;
                for (int i = 0; i < CellPermises.Count; i++)
                {
                    if (CellPermises[i].F < CellPermises[index].F)
                        index = i;
                    if (CellPermises[i].F != CellPermises[index].F) continue;
                    if (CellPermises[i].G > CellPermises[index].G)
                        index = i;
                    if (CellPermises[i].G == CellPermises[index].G)
                        index = i;
                }

                Cell Actual = CellPermises[index];
        ...
        public void SetCellArray(Cell[] c)
        {
            cells = c;
        }
